...
from momograd.x.util import append_to_file
...
fn main() raises:
    ...
    var input = InlinedFixedVector[ValueXList](N_SAMPLES)
    var yb = InlinedFixedVector[ValueX](N_SAMPLES)

    for i in range(N_SAMPLES):
        input[i] = ValueXList(npx[i][0].to_float64(), npx[i][1].to_float64())
       
        yb[i] = 2 * ValueX(npy[i].to_float64()) - 1

    var scores = ValueXList(N_SAMPLES)

    var data_loss = ValueX(0)
    var reg_loss = ValueX(0)
